...
function draw() {
  ...
  if(estado == "fondo"){
    if(inicioElSonido){
      ...
      image(trazos[cual], x, y2, 80, 200);
      image(trazos[cual], x, y3, 80, 200);
      Capa1++;
    }
  }
 if(finDelSonido){
  ...
  if(inicioElSonido){
    ...
    posiciones[Capa3][0] = int(x); // Almacenar la posición ocupada
     posiciones[Capa3][1] = int(y);
      ...
      image(trazos[cual], x, y2, 40, 60);
      image(trazos[cual], x, y3, 40, 60);
      Capa3++;
  }
}
...
if(!haySonido){
  ...
  if(inicioElSonido){
    if (Capa5 < 15) {
      ...
      image(trazos[cual], x, y2, 20, 100);
      image(trazos[cual], x, y3, 20, 100);
      Capa5++;  
    } 
}
